function savePath = export(file, filePath, folderName, areaThreshold, smoothSize)
    ...
    if sizeTable < 2^20
        writetable(dataTable, [savePath '\Output_' folderName '.csv']);
    else
        if rem(sizeTable,2) == 1
            writetable(dataTable(1:(sizeTable-1)/2, :), [savePath '\Output_' folderName '_1.csv']);
            writetable(dataTable(((sizeTable-1)/2+1):sizeTable, :), [savePath '\Output_' folderName '_2.csv']);
        else
            writetable(dataTable(1:sizeTable/2, :), [savePath '\Output_' folderName '_1.csv']);
            writetable(dataTable((sizeTable/2+1):sizeTable, :), [savePath '\Output_' folderName '_2.csv']);
        end
    end
